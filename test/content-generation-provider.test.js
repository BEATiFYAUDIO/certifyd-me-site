...
async function writeTrendSources(config, sourceItems) {
  await fs.mkdir(path.join(config.agentRoot, 'dashboard/trends'), { recursive: true });
  await fs.writeFile(path.join(config.agentRoot, 'dashboard/trends/trend-state.json'), `${JSON.stringify({ sourceItems }, null, 2)}\n`);
}

async function writeAudiomackBrainFixture(config) {
  const records = [
    ['content-agent/knowledge/ecosystem.md', [
      ...
      'This architecture is intended to reduce creator dependency on centralized platforms without claiming Certifyd is fully decentralized or company-independent.',
    ].join('\n')],
    ['content-agent/knowledge/capabilities/release-records.md', [
      '# Release Records',
      '',
      'APPROVED',
      '',
    ...
    ['content-agent/knowledge/capabilities/network-distribution.md', [
      ...
      '## Safe Wording',
      '',
      '- Network distribution exists to reduce dependency on a single centralized platform model and expand creator/operator participation.',
      '',
      '## Qualified Claims',
      '',
      '- Certifyd Network is approved as a distributed infrastructure layer, but exact Core-to-node runtime and governance are unresolved.',
    ].join('\n')],
  ];
...
function whyCertifydContext({ title, summary, thesis, concept, sourceConnection, canonicalThesis = null }) {
  return {
    sourceRecords: [{
...
test('OpenAI final writing instructions discourage validator-facing defensive prose and forced Certifyd sections', async () => {
  ...
  assert.match(finalInstructionText, /Use source-only reasoning as advisory analysis/i);
  assert.match(finalInstructionText, /only when doing so reinforces the canonical thesis/i);
  assert.match(finalInstructionText, /Distinguish news facts, the first structural interpretation, the deeper architectural change, and supporting operational consequences/i);
  assert.match(finalInstructionText, /After identifying the first structural interpretation, ask what larger change makes that interpretation possible or necessary/i);
  assert.match(finalInstructionText, /Do not confuse operational implications such as metadata, attribution, records, transparency, workflow continuity, release context, settlement, discovery, commerce, permissions, or administrative complexity with the deepest thesis/i);
  ...
  assert.match(finalInstructionText, /what used to sit outside the product but is now moving inside it/i);
  assert.match(finalInstructionText, /what used to happen after the fact but is now moving earlier/i);
  assert.match(finalInstructionText, /machine-operable/i);
  assert.match(finalInstructionText, /boundary between previously separate systems is disappearing/i);
  assert.match(finalInstructionText, /social identity becoming financial identity/i);
  ...
  assert.match(finalInstructionText, /ask whether the function actually needs a centralized intermediary/i);
  assert.match(finalInstructionText, /creator-operated infrastructure changes the control relationship/i);
  ...
  assert.match(finalInstructionText, /select only the Brain capabilities needed to illuminate the thesis/i);
  assert.match(finalInstructionText, /current approach or solution -> what it improves -> what dependency, limitation, fragmentation or control problem remains -> selected Certifyd approach -> the practical difference for the creator/i);
  assert.match(finalInstructionText, /This is critical analysis, not hostility/i);
  assert.match(finalInstructionText, /Do not spend repeated language reassuring the reader that the outside product is useful/i);
  assert.match(finalInstructionText, /follow the architecture to the point where its control, context or portability stops/i);
  assert.match(finalInstructionText, /Do not position Certifyd as merely another tool beside the source company/i);
  assert.match(finalInstructionText, /tools used by creators and infrastructure operated or controlled by creators/i);
  assert.match(finalInstructionText, /Do not default to conclusions such as “the future needs both useful services and durable creator-controlled infrastructure,” “platforms and Certifyd can work together,” or “Certifyd complements these tools.”/i);
  assert.match(finalInstructionText, /why does this function need a centralized company in the middle at all/i);
  assert.match(finalInstructionText, /what part of the creator operation is being pulled into the service/i);
  assert.match(finalInstructionText, /identity, catalog context, publishing, analytics, promotion, payout access, commerce, permissions, discovery, fan relationships, or operating records/i);
  assert.match(finalInstructionText, /can deepen dependency by moving more of the creator operation into infrastructure the creator does not operate/i);
  assert.match(finalInstructionText, /Certifyd Core as a different network architecture/i);
  assert.match(finalInstructionText, /can originate from creator-operated infrastructure instead of requiring a centralized platform to own the operating environment/i);
  assert.match(finalInstructionText, /Avoid vague Certifyd conclusions such as/i);
  assert.match(finalInstructionText, /avoid generic product pitching, feature inventory, product documentation or capability lists/i);
  assert.match(finalInstructionText, /Do not automatically create sections titled “The Certifyd perspective,” “How this connects to Certifyd,” or “Why this matters to Certifyd” unless that structure is editorially natural/i);
  assert.match(finalInstructionText, /Do not mechanically mention every selected Brain record/i);
  assert.match(finalInstructionText, /Do not add unrelated surfaces such as discovery, Awards, commerce, profiles, or payouts unless they materially support the article’s argument/i);
  assert.match(finalInstructionText, /For a substantial multi-source opportunity, do not end after summarizing the announcement, identifying the first structural observation, and listing several operational consequences/i);
  assert.match(finalInstructionText, /what deeper change it reveals/i);
  assert.match(finalInstructionText, /where relevant Certifyd architecture extends the argument, if genuinely relevant/i);
  assert.match(finalInstructionText, /Editorial inference may be a reasoned conclusion drawn from supported facts/i);
  ...
  assert.doesNotMatch(finalInstructionText, /permission by design/i);
  assert.doesNotMatch(finalInstructionText, /permission is moving upstream/i);
  assert.doesNotMatch(finalInstructionText, /authorization becomes infrastructure/i);
...
test('final writing instructions require why-Certifyd architectural need before feature inventory', async () => {
  ...
  assert.match(finalInstructionText, /current approach or solution -> what it improves -> what dependency, limitation, fragmentation or control problem remains -> selected Certifyd approach -> the practical difference for the creator/i);
  assert.match(finalInstructionText, /State what it does, then follow the architecture to the point where its control, context or portability stops/i);
  assert.match(finalInstructionText, /infrastructure operated or controlled by creators/i);
  assert.match(finalInstructionText, /ask whether the function actually needs a centralized intermediary/i);
  assert.match(finalInstructionText, /why does this function need a centralized company in the middle at all/i);
  assert.match(finalInstructionText, /creator operation is being pulled into the service/i);
  assert.match(finalInstructionText, /creator-operated infrastructure instead of requiring a centralized platform to own the operating environment/i);
  assert.match(finalInstructionText, /Replace them with the specific supported capability and the concrete architectural difference it creates in this story/i);
  ...
  assert.match(finalInstructionText, /CANONICAL EDITORIAL THESIS/i);
  assert.match(finalInstructionText, /source-of-truth for the article’s structural claim/i);
  assert.doesNotMatch(finalInstructionText, /may contain zero Certifyd product references/i);
  assert.doesNotMatch(finalInstructionText, /source-backed article may contain zero/i);
});

test('canonical thesis frames Audiomack as centralized creator operating functions, not generic commerce', async () => {
  const config = await makeConfig();
  await writeTrendSources(config, [{
    id: 'source-audiomack-pro',
    publisher: 'Music Business Worldwide',
    publishedAt: '2026-09-16T12:00:00.000Z',
    title: 'Audiomack Pro rolls out globally including Song Pulse feature',
    summary: 'Audiomack Pro is rolling out globally with Song Pulse analytics, profile verification, promotional tools, monetization access and payout-related features for artists using the streaming service.',
    articleUrl: 'https://example.test/audiomack-pro',
    categories: ['Music Business'],
    certifydRelevanceScore: 13,
  }]);

  const context = await makeContext(config, {
    topic: 'Write a Certifyd article about: Audiomack Pro rolls out globally including Song Pulse feature. Use this angle: commerce, receipts and transparent records.',
    trendSourceItemIds: 'source-audiomack-pro',
  });

  const thesis = context.editorialBrief.canonicalThesis;
  assert.equal(thesis.mode, 'centralized-intermediary');
  assert.match(thesis.industryDevelopment, /Audiomack is moving analytics, promotion, verification and payout access/i);
  assert.match(thesis.structuralChange, /centralized inside Audiomack/i);
  assert.match(thesis.dependency, /dependent on infrastructure, rules and relationships controlled by Audiomack/i);
  assert.match(thesis.architecturalQuestion, /Why must these creator functions require a centralized intermediary/i);
  assert.match(thesis.certifydRelevance, /opposite architectural direction/i);
  assert.match(thesis.creatorConsequence, /toward operating part of the infrastructure themselves/i);
});

test('Audiomack-like final writer context keeps architecture source alongside current capability facts', async () => {
  const calls = [];
  const config = await makeConfig();
  await writeAudiomackBrainFixture(config);
  await writeTrendSources(config, [{
    id: 'source-audiomack-pro',
    publisher: 'Music Ally',
    publishedAt: '2026-09-18T12:00:00.000Z',
    title: 'Audiomack Pro rolls out globally including Song Pulse feature',
    summary: 'Audiomack Pro is rolling out globally with Song Pulse analytics, profile verification, promotional tools, monetization access, payout access and fan engagement tools for artists using the streaming service.',
    articleUrl: 'https://example.test/audiomack-pro',
    categories: ['Music Business'],
    certifydRelevanceScore: 13,
  }]);
  ...
  const context = await makeContext(config, {
    topic: 'Write a Certifyd article about: Audiomack Pro rolls out globally including Song Pulse feature.',
    trendSourceItemIds: 'source-audiomack-pro',
  });
  const article = sourceBackedArticleWithoutFixtureFooter(context, [
    ...
    'The article therefore keeps the source fact, the current Certifyd capability and the future architectural direction separate. Audiomack can ship useful artist tools, while Certifyd can still argue for an operating layer that starts with the creator instead of with the platform.',
  ], {
    claims: [{
      text: 'Certifyd Core supports release records.',
      sourceIds: ['brain:capabilities/release-records'],
      confidence: 'supported',
    }],
  });
  const provider = new OpenAIGenerationProvider(config, {
    openaiClient: mockOpenAIClient({
      ...
      reasoning: validReasoning({
        ...
        hiddenQuestion: 'Why must these creator functions require a centralized intermediary?',
  ...
  await provider.generateArticle({
    actorEmail: 'writer@example.test',
    topic: context.editorialBrief.primaryEvent,
    audience: 'Creators',
    objective: 'Explain the source story.',
    trendSourceItemIds: 'source-audiomack-pro',
  }, context);

  assert.equal(context.editorialBrief.canonicalThesis.mode, 'centralized-intermediary');
  assert.match(calls[1].input, /CANONICAL EDITORIAL THESIS/);
  assert.match(calls[1].input, /Why must these creator functions require a centralized intermediary/i);
  assert.match(calls[1].input, /brain:capabilities\/release-records/);
  assert.match(calls[1].input, /Certifyd Core supports release records/i);
  assert.match(calls[1].input, /brain:ecosystem|brain:capabilities\/network-distribution|brain:facts\/approved-public-claims/);
  assert.match(calls[1].input, /reduce creator dependency on centralized platforms|reduce dependency on a single centralized platform model|creator-operated infrastructure/i);
  assert.match(calls[1].input, /For centralized-intermediary canonical theses, carry the same canonical argument/i);
});

test('canonical thesis keeps non-platform rights story source-specific', async () => {
  const config = await makeConfig();
  await writeTrendSources(config, [{
    id: 'source-afm-rights',
    publisher: 'Music Business Worldwide',
    publishedAt: '2026-09-14T12:00:00.000Z',
    title: 'AFM files opposition in Universal and Warner AI licensing dispute',
    summary: 'A musicians union filing challenges AI licensing arguments involving recordings, permissions, performer terms and alleged commercial exploitation of recorded work.',
    articleUrl: 'https://example.test/afm-ai-rights',
    categories: ['Music Rights'],
    certifydRelevanceScore: 13,
  }]);

  const context = await makeContext(config, {
    topic: 'Write a Certifyd article about: AFM files opposition in Universal and Warner AI licensing dispute.',
    trendSourceItemIds: 'source-afm-rights',
  });

  const thesis = context.editorialBrief.canonicalThesis;
  assert.equal(thesis.mode, 'source-specific');
  assert.match(thesis.architecturalQuestion, /dependency, control problem, or infrastructure gap/i);
  assert.doesNotMatch(thesis.structuralChange, /centralized inside/i);
});
...
test('why-Certifyd validation rejects centralized-platform story that only complements the intermediary', () => {
  ...
  const article = sourceBackedArticleWithoutFixtureFooter(context, [
    ...
    'That creates a dependency problem because artist workflows can sit inside infrastructure the platform controls.',
    'Certifyd matters because Certifyd Core supports identity, publishing and commerce infrastructure that gives creators a more durable foundation alongside useful platform tools.',
    'That foundation can preserve work and relationship context across services while Audiomack continues to improve the artist dashboard.',
    'The article still has to explain the architectural consequence rather than merely praising the dashboard or naming a creator-controlled layer. Artists can benefit from better reporting, promotion and monetization access while still having the operating environment defined by the service that owns the account.',
    'If the Certifyd passage only says the creator needs a durable foundation beside the platform, it has not answered the canonical question. The missing argument is whether analytics, verification, payout access and fan context should have to live in the outside account before the creator business becomes legible.',
    'That distinction matters because the news is not just that one service added useful artist features. The deeper issue is where the operating record lives after those features become normal. A draft can be long enough and still miss the point if it leaves the platform as the permanent center and treats creator-controlled infrastructure as a helpful add-on around the edges.',
  ]);
...
test('why-Certifyd validation accepts centralized-platform story that answers intermediary displacement thesis', () => {
  ...
  const article = sourceBackedArticleWithoutFixtureFooter(context, [
    'Audiomack Pro puts analytics, promotion, payout access and verification inside the platform account.',
    'That creates a dependency problem because the artist operating record can sit inside infrastructure the platform controls.',
...
test('why-Certifyd validation blocks unsupported completed platform-replacement claims', () => {
  const context = whyCertifydContext({
    title: 'Audiomack Pro expands artist tools',
    summary: 'The source story says Audiomack Pro adds analytics, promotion, payout access and verification for artists inside the streaming service.',
    thesis: 'More artist workflow is moving inside centralized platform infrastructure.',
  ...
  const article = sourceBackedArticleWithoutFixtureFooter(context, [
    ...
    'Certifyd matters to that specific problem because its architecture starts from creator-operated infrastructure rather than treating the platform account as the source of the artist business record.',
    'That changes the control relationship and makes possible a supported path where creator functions do not have to require a centralized intermediary as the necessary home for the artist business.',
    'Certifyd already replaces Audiomack analytics, promotion and payout systems for artists.',
    'That unsupported completed-functionality claim is exactly the boundary the generator must not cross.',
  ]);
  assert.throws(() => validateGeneratedArticle(article, context), /unsupported external Certifyd adoption claims/i);
});
...
test('OpenAI Certifyd architectural relevance repair passes and returns revised draft', async () => {
  ...
  assert.match(calls[2].input, /restore the missing architectural relationship/i);
  assert.match(calls[2].input, /why that function should not have to require a centralized company in the middle/i);
...
test('OpenAI analytical vocabulary in reasoning is allowed without becoming Brain evidence', async () => {
  ...
  assert.match(calls[1].input, /ALLOWED_BRAIN_SOURCE_IDS:\n\[\n  "brain:facts\/approved-public-claims"\n\]/);
  assert.match(calls[1].input, /Approved Public Claims/i);
  assert.doesNotMatch(calls[1].input, /Royalty context.*sourceConnection/i);
});
