...
function sourceStoryCard(story, csrf = '', canCreate = false, sourceDraftIndex = new Map()) {
  ...
  const actions = existing
    ? `<div class="source-story-actions"><span class="pill good">Draft exists</span><a class="ghost" href="/app/content/articles/${encodeURIComponent(existing.runId)}">Open draft</a>${readOriginal}</div>`
