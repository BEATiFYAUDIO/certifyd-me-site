...
import { ensureImageBriefForRun } from './image-generation.js';
...
async function handleAction(req, res, url, ctx) {
  ...
  else if (action.endsWith('/publishing/image-brief-reset')) { needs('content.article.edit'); result = await ctx.actions.resetImageBrief({ actor: ctx.user, runId: form.get('runId') }); }
  ...
  if (action.endsWith('/publishing/cover') || action.endsWith('/publishing/cover-upload') || action.endsWith('/publishing/image-generate') || action.endsWith('/publishing/image-brief-reset') || action.endsWith('/publishing/image-approve')) {
...
function sourceStoryCard(story, csrf = '', canCreate = false, sourceDraftIndex = new Map()) {
  ...
  const readOriginal = story.sourceUrl
    ? `<a class="ghost" href="${escapeHtml(story.sourceUrl)}" rel="noreferrer" target="_blank">Read original ↗</a>`
    : '<span class="muted">No original source</span>';
...
async function renderArticle(ctx, runId, csrf) {
  validateRunId(runId);
  await ensureImageBriefForRun(ctx.config, ctx.runRepo, runId);
