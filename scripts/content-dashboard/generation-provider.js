...
function buildArticleSystemInstruction() {
  return [
    ...
    'A reasoned editorial inference may be drawn from supported facts, but it must read as analysis rather than falsely attributed reporting.',
    'Write like an informed technology/music-business publication, not a compliance memo.',
    'Use plain, direct language for intelligent creators, musicians, producers and general music-business readers. Aim roughly for Grade 8 to 10 readability without simplifying the underlying reasoning.',
    'Prefer familiar words, concrete statements, active voice and shorter sentence structures over academic, corporate, policy, investor or strategy-memo language.',
    'Preserve necessary industry terminology, factual qualifications and technical precision. When an idea can be expressed accurately in simpler language, use the simpler language.',
    'Avoid elevated strategy phrases such as “The strategic direction is this,” “The more consequential,” “The significance for,” “This raises the stakes around,” “increasingly inseparable from,” “the broader implication is,” “This development underscores,” or “As the industry evolves.”',
    'Avoid abstract noun chains such as “verification-led economics,” “creator workflow context,” or “participation architecture” when a simpler concrete explanation would be accurate.',
...
function buildArticlePrompt(input, groundedContext, reasoning, writingContext) {
  ...
  return [
    ...
    '- Ask what is changing in the architecture of the market, product, rights system, business relationship, or creator relationship.',
    '- Ask what used to sit outside the product but is now moving inside it; what used to happen after the fact but is now moving earlier; what used to be a policy, contract, database entry, or business rule but increasingly needs to become machine-operable; what boundary between previously separate systems is disappearing; and what industry assumption is being replaced.',
    '- The final thesis should usually identify the larger architectural change first, then use operational consequences such as provenance, attribution, metadata, release records, rights context, settlement, discovery, or commerce to show why that thesis matters.',
    '- General depth examples, not templates: if a social network adds payments, the deeper change may be social identity becoming financial identity; if a distributor adds creation tools, the deeper change may be distribution infrastructure moving upstream into creation; if AI licensing connects model participation and release, discover the deeper change from the evidence rather than reusing supplied wording.',
    ...
    depthGuidance,
    '- For a substantial multi-source opportunity, do not end after summarizing the announcement, identifying the first structural observation, and listing several operational consequences. Develop the deeper thesis and use the rest of the article to test and explain it.',
    '- A strong article should generally answer what happened; what people immediately think it means; what deeper change it reveals; why that change matters; what new infrastructure or behavior it creates demand for; where relevant Certifyd architecture extends the argument, if genuinely relevant; and what remains unresolved. Do not force sections matching those questions.',
    '- Missing terms, unanswered questions and uncertainty can be factual caveats, but do not let caveats replace the thesis. “Terms remain unclear” is usually not an editorial conclusion.',
    '- Facts must be supported by source material or selected Brain. Editorial inference may be a reasoned conclusion drawn from supported facts. Certifyd facts must be supported by selected Brain. External adoption claims cannot be established by Brain.',
    '- Keep the reasoning deep but the language simple. Write for an intelligent creator, musician, producer or music-business reader who should not need a technology, policy, finance or academic background to follow the article.',
    '- Prefer plain sentences such as “If payouts depend on whether someone actually listened, then how you prove a real listen suddenly matters” over elevated strategy or policy language.',
    '- Use necessary terms of art when they carry real meaning, but explain the idea in normal editorial prose around them.',
...
function requiresCertifydArchitecturalReasoning(windowText) {
  return /\bcertifyd['’]s perspective\b/.test(windowText)
    || /\bcertifyd\b[^.]{0,180}\b(?:supports|provides|offers|includes|powers|maintains|enables|preserves)\b/.test(windowText)
    || /\bcertifyd\b[^.]{0,180}\b(?:release records?|catalog(?:-management)?|catalog context|work context|attribution context|public records?|capabilit(?:y|ies))\b/.test(windowText);
}

function hasCertifydDependencyReasoning(windowText) {
  return /\b(?:platform|platforms|third-party|third party|intermediary|intermediaries|distributor|distributors|label|labels|pro|cmo|ai product|ai products|marketplace|marketplaces|downstream systems?|outside systems?|other systems?|elsewhere|dependent|depends|dependency|fragmented|fragmentation|fragile|rebuild|recreated|recreate|cede|controlled elsewhere|left entirely|left only|exists only|lives only|sits inside|inside every|inside another|inside someone else's|not control|do not control|cannot carry|unable to carry|without that continuity|without continuity|without an independent|without creator-controlled|rather than only|rather than starting)\b/.test(windowText);
}

function hasCertifydControlChangeReasoning(windowText) {
  return /\b(?:creator-controlled|creator controlled|creator-operated|creator operated|independent|portable|persistent|durable|own record|own layer|starting point|starting place|source context|source of|originates with the creator|originates with creators|creator maintains|creators maintain|creator can maintain|creators can maintain|maintained by the creator|controlled by the creator|infrastructure layer)\b/.test(windowText);
}
...
function isGenericCertifydFeatureList(text) {
  const windows = [...String(text || '').matchAll(/\bcertifyd\b/g)].map((match) => {
    const value = String(text || '');
    const start = Math.max(0, match.index - 120);
    return value.slice(start, match.index + 360);
  });
  ...
  return windows.some((windowText) => {
    ...
    const hasCreatorControl = hasCertifydControlChangeReasoning(windowText);
    const hasOutcome = hasCertifydOutcomeReasoning(windowText);
    const hasConcreteCertifydCapabilityClaim = shallowCapabilitySentence
      || /\bcertifyd['’]s perspective\b/i.test(windowText)
      || /\bcertifyd\b[^.!?]{0,180}\b(?:release records?|catalog(?:-management)?|catalog context|work context|attribution context|public records?|capabilit(?:y|ies))\b/i.test(windowText);
    return (hasConcreteCertifydCapabilityClaim && featureHits >= 3 && hasCapabilityVerb && !(hasDependency && hasCreatorControl && hasOutcome))
