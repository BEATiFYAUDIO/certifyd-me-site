...
function homepageStyles() {
  return `${HOME_CSS_START}
    .social-home-section{margin-top:clamp(30px,5vw,64px)}
    .social-home-panel{border:1px solid var(--border);border-radius:32px;background:linear-gradient(145deg,rgba(255,255,255,.075),rgba(255,255,255,.025)),rgba(5,24,38,.38);box-shadow:var(--shadow);padding:clamp(22px,4vw,42px)}
    .social-home-kicker{display:flex;align-items:baseline;gap:8px;flex-wrap:wrap;margin:0 0 12px}
    .social-home-new{display:inline-flex;align-items:center;border:1px solid rgba(255,190,94,.5);border-radius:999px;background:rgba(255,138,0,.1);color:var(--certifyd-orange);font-size:9px;font-weight:900;letter-spacing:.12em;line-height:1;padding:4px 7px;text-transform:uppercase;transform:translateY(-1px)}
    .social-home-head{max-width:880px;margin-bottom:24px}
    .social-home-head h2{margin:0;font-size:clamp(34px,5vw,74px);line-height:.9;letter-spacing:-.065em;color:var(--certifyd-text)}
    .social-home-head p{max-width:760px;margin:14px 0 0;color:var(--text-secondary);font-size:clamp(16px,1.5vw,20px)}
    .social-home-head strong{color:var(--certifyd-text)}
    .social-feed-frame{overflow:hidden;border:1px solid var(--glass-border);border-radius:24px;background:rgba(2,12,22,.58);box-shadow:inset 0 1px 0 rgba(255,255,255,.06)}
    .social-feed-frame iframe{display:block;width:100%;height:clamp(520px,72vh,820px);border:0;background:#062133}
    .social-home-actions{display:flex;justify-content:flex-start;margin-top:18px}
    ...
    @media (max-width:860px){.social-home-panel,.blog-home-panel{border-radius:24px}.social-feed-frame{border-radius:20px}.social-feed-frame iframe{height:min(72vh,640px)}.blog-home-head{align-items:start;flex-direction:column}.blog-home-grid{grid-template-columns:1fr}.home-blog-body{padding:16px}}
${HOME_CSS_END}`;
}
...
function homepageSection(articles) {
  ...
  return `${HOME_SECTION_START}
    <section class="wrap social-home-section" aria-labelledby="social-home-heading">
      <div class="social-home-panel">
        <div class="social-home-head">
          <p class="social-home-kicker"><span class="live-profile-eyebrow">CERTIFYD POSTS</span><span class="social-home-new">NEW</span></p>
          <h2 id="social-home-heading">See What Creators Are Saying.</h2>
          <p>Posts and conversations from creators across the Certifyd network — <strong>authorship proven by cryptography.</strong></p>
          <p>Every Certifyd Post is cryptographically signed and tied to a verified network participant, helping keep conversations accountable, authentic, and resistant to impersonation.</p>
        </div>
        <div class="social-feed-frame" role="region" aria-label="Public Certifyd Social feed">
          <iframe src="https://social.certifyd.me/#feed" title="Public Certifyd Social feed" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" sandbox="allow-scripts allow-same-origin allow-popups"></iframe>
        </div>
        <div class="social-home-actions">
          <a class="btn primary" href="https://social.certifyd.me/" target="_blank" rel="noreferrer">Explore Certifyd Social →</a>
        </div>
      </div>
    </section>
